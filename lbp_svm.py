...
if __name__ == "__main__":
    ...
    for feat_path in glob.glob(os.path.join(train_feat_path2, '*.feat')):#在config.cfg改
    ...
    if clf_type is 'LIN_SVM':
        #clf = LinearSVC(C=1)#线性分类器，c越大效果越好，但是可能过拟合
        ...
        joblib.dump(clf, model_path2_3)#路径在config保存
        clf = joblib.load(model_path2_3)#加载

        print ("Classifier saved to {}".format(model_path2_3))
        for feat_path in glob.glob(os.path.join(test_feat_path2, '*.feat')):#开始测试
            total += 1#到一万截至
